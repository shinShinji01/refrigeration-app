...
export const useVertexDrag = ({ dispatch, svgRef, viewBox }: UseVertexDragArgs) => {
  ...
  const getVertexHandlers = (index: number) => ({
    onPointerDown: (event: ReactPointerEvent<SVGCircleElement>) => {
      ...
      // Захватываем указатель: независимо от размера хит-зоны палец/мышь может увести
      // за её пределы уже в первые пиксели драга (быстрое движение, дрожь руки). Без
      // capture move/up после этого ретаргетятся по обычному hit-testing на то, что сейчас
      // под курсором — onPointerUp по этому кружку никогда не долетит, draggedIndexRef не
      // очистится, и следующий случайный pointermove над любой другой вершиной молча
      // передвинет чужую точку по устаревшему индексу.
